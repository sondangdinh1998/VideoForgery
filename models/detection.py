...
def check_keys(model, pretrained_state_dict):
    ...
    assert len(used_pretrained_keys) > 0, "NONE from pretrained checkpoint"
    return True
...
def load_detector(model_path, device="cpu", network="mobilenet"):
...
def batch_detect(net, images, device, is_tensor=False):
    with torch.no_grad():
        ...
        img = img.to(device)
        mean = (
            torch.as_tensor([104, 117, 123], dtype=img.dtype, device=device)
            .unsqueeze(0)
            .unsqueeze(0)
            .unsqueeze(0)
        )
...
def get_valid_faces(detect_results, max_count, threshold, at_least=False):
    new_results = []
    for faces in detect_results:
        ...
        for j, face in enumerate(faces):
            if face[-1] < threshold and not (j == 0 and at_least):
                continue
            box, lm, score = face
            box = box.astype(int)
            lm = lm.astype(int)
